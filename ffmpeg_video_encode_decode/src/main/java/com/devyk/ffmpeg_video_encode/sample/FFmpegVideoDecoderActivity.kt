...
public class FFmpegVideoDecoderActivity : BaseActivity<Int>() {
    ...
    private var OUT_YUV_PATH = "sdcard/aveditor/text.yuv"
//    private var IN_H264_PATH = "sdcard/avsample/352_288_i420.h264"
    private var IN_MP4_PATH = "sdcard/aveditor/123.h264"
    ...
    override fun init() {
        ...
        btn_start.setOnClickListener {
            var isSuccess = nativeFFmpegVideoApi.init(IN_MP4_PATH, OUT_YUV_PATH, 720, 1280, 1000);
            if (isSuccess == 1) {
                startTime(timer_)
                nativeFFmpegVideoApi.start()
                return@setOnClickListener
            }
            Toast.makeText(applicationContext,"解码失败",Toast.LENGTH_LONG).show()
        }
