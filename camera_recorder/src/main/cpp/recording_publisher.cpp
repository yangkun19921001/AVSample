...
void RecordingPublisher::registerFillAACPacketCallback(
        int (*fill_aac_packet_callback)(AudioPacket **, void *context), void *context) {
...
int RecordingPublisher::detectTimeout() {
    if (platform_4_live::getCurrentTimeMills() - sendLatestFrameTimemills > publishTimeout) {
        int queueSize = AVPacketPool::GetInstance()->getRecordingVideoPacketQueueSize();
...
int fill_aac_packet(AudioPacket **pAudioPacket, void *context) {

    return AVPacketPool::GetInstance()->getAudioPacket(pAudioPacket);

}

int fill_packet_frame(VideoPacket **pVideoPacket, void *context) {
...
int RecordingPublisher::init(char *videoOutputURI,
    ...
    registerFillAACPacketCallback(fill_aac_packet, this);
    registerFillVideoPacketCallback(fill_packet_frame, this);
...
int RecordingPublisher::encode() {
    while (true) {
        ...
        if (verboseOn) {
            LOGE("video_time is %lf, audio_time is %f", video_time, audio_time);
        }
        /* write interleaved audio and video frames */
        if (!video_st || (video_st && audio_st && audio_time < video_time)) {
            LOGE("write_audio_frame");
            ret = write_audio_frame(oc, audio_st);
        } else if (video_st && oc && video_st) {
...
int RecordingPublisher::buildAudioStream(char *audio_codec_name) {
    ...
    audio_st = add_stream(oc, &audio_codec, AV_CODEC_ID_AAC, audio_codec_name);
...
int RecordingPublisher::interleavedWriteFrame(AVFormatContext *s, AVPacket *pkt) {
    ...
    if (ret == 0) {
        LOGE("av_interleaved_write_frame success!");
    }
    return ret;
}
...
void *start_(void *pVoid) {
    RecordingPublisher *recording = static_cast<RecordingPublisher *>(pVoid);
    recording->encode();
    return 0;
}

void RecordingPublisher::start() {
    isRuning = true;
    pthread_create(&id, 0, start_, this);
